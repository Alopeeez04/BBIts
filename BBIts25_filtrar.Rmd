...
ggplot(cluster_summary %>% filter(recidiva == 1),
  ...
  scale_fill_gradient(low = "white", high = "red", name = "Proporción") +
...
# 1. Subset: pacients sense recidiva
non_relapsed <- data_rec %>%
  filter(recidiva == 0)

# 2. Variables d'interès
vars_sig <- c("Grado", "libre_enferm", "beta_cateninap", "mlh1",
              "grupo_de_riesgo_definitivo", "Tributaria_a_Radioterapia",
              "afectacion_linf", "grado_histologi", "AP_centinela_pelvico",
              "AP_ganPelv", "tipo_histologico")

# 3. Preparar freqüències per cluster i estat vital
heatmap_data <- non_relapsed %>%
  select(cluster, cluster_label, recidiva_exitus, all_of(vars_sig)) %>%
  mutate(death_status = ifelse(recidiva_exitus == 1, "Morts", "Vius")) %>%
...
# 4. Diccionari de traducció al català
labels_vars <- c(
  Grado = "Grau tumoral",
  ...
  AP_centinela_pelvico = "Gangli centinella pèlvic",
  AP_ganPelv = "Gangli pèlvic",
  tipo_histologico = "Tipus histològic"
)
...
# 6. Gràfic heatmap
ggplot(heatmap_data, aes(x = variable, y = cluster_label, fill = freq)) +
  geom_tile(color = "black") +
  scale_fill_gradient(low = "white", high = "red", name = "Proporció") +
  facet_wrap(~death_status) +
  theme_minimal(base_size = 14) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(title = "Distribució de variables vius/morts en pacients sense recidiva",
       x = "Variable",
       y = "Cluster")
