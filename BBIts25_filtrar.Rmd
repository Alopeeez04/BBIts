...
# 4. Plot: two heatmaps side by side
ggplot(heatmap_data, aes(x = variable, y = cluster_label, fill = freq)) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(title = "Distribució de Variables vius/morts AMB PACIENTS NO RECESIVA",
       x = "Variable",
       y = "Cluster")
...
# 1. Subset: patients who relapsed
relapsed <- data_rec %>%
  filter(recidiva == 1)
...
ggplot(heatmap_data, aes(x = variable, y = cluster_label, fill = freq)) +
  ...
  theme_minimal(base_size = 14) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(title = "Distribució de Variables vius/morts AMB PACIENTS RECESIVA",
       x = "Variable",
       y = "Cluster")
